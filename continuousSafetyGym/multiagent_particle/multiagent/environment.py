...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, render_mode, reset_callback=None, reward_callback=None,
        ...
        for agent in self.agents:
            ...
            if self.constraint_callback is not None:
                ...
                self.constraint_space.append(spaces.Box(low=-np.inf, high=+np.inf, shape=(constraint_dim,), dtype=np.float64))
        ...
        low = np.concatenate([sub_space.low for sub_space in self.observation_space])
        high = np.concatenate([sub_space.high for sub_space in self.observation_space])
        obs_shape = self.reset()[0].shape
        self.observation_space = spaces.Box(low=low, high=high, shape=obs_shape, dtype=np.float64)

        low = np.concatenate([sub_space.low for sub_space in self.action_space])
        high = np.concatenate([sub_space.high for sub_space in self.action_space])
        act_shape = np.concatenate([sub_space.sample() for sub_space in self.action_space]).shape
        self.sub_action_spaces = self.action_space
        self.action_space = spaces.Box(low=low, high=high, shape=act_shape, dtype=np.float64)

        low = np.concatenate([sub_space.low for sub_space in self.constraint_space])
        high = np.concatenate([sub_space.high for sub_space in self.constraint_space])
        constraint_shape = np.concatenate([sub_space.sample() for sub_space in self.constraint_space]).shape
    ...
    def step(self, action):
        ...
        action_n = action.copy()
        action_n = np.split(action_n, self.n)

        self.agents = self.world.policy_agents
        # set action for each agent
        for i, agent in enumerate(self.agents):
            self._set_action(action_n[i], agent, self.sub_action_spaces[i])
        # advance world state
        self.world.step()
    ...
    def reset(self, *, seed: Optional[int] = None, options={}):
        ...
        for agent in self.agents:
            ...
            constraint_n.append(self._get_constraints(agent))
        obs_n = np.concatenate(obs_n)
        return obs_n, {'cost': np.concatenate(constraint_n)}
    ...
    def get_env_parameters(self):
        ...
        params["constraint_dim"] = self.sub_constraint_space[0].shape[0]
        return params
