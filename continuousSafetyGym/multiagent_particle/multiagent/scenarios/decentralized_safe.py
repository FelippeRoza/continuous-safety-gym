import numpy as np
from continuousSafetyGym.multiagent_particle.multiagent.core import World, Agent, Landmark
from continuousSafetyGym.multiagent_particle.multiagent.scenario import BaseScenario
# import ipdb
...
class Scenario(BaseScenario):
    ...
    def constraints(self, agent, world):
        ...
        # no collision if c < 0, collision if c >= 0

        other_agents = [a for a in world.agents if a is not agent]

        collision_signals = np.zeros(len(world.agents) - 1)
        for i, other in enumerate(other_agents):
            collision_signals[i] = - np.linalg.norm(other.state.p_pos - agent.state.p_pos) + agent.size + other.size
