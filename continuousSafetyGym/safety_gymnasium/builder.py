...
from continuousSafetyGym.safety_gymnasium import tasks
from continuousSafetyGym.safety_gymnasium.bases.base_task import BaseTask
from continuousSafetyGym.safety_gymnasium.utils.common_utils import ResamplingError, quat2zalign
from continuousSafetyGym.safety_gymnasium.utils.task_utils import get_task_class_name
...
class Builder(gymnasium.Env, gymnasium.utils.EzPickle):
    ...
    - :attr:`config` (dict): Pre-defined configuration of the environment, which is passed via
      :meth:`continuousSafetyGym.safety_gymnasium.register()`.
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        self.constraint_space = gymnasium.spaces.Box(low=-np.inf, high=+np.inf, shape=(1,), dtype=np.float64)
    ...
    def reset(
        ...
        *,
        ...
        cost = self._cost()
        assert cost['cost_sum'] <= 0, f'World has positive starting cost! {cost}'
        # Reset stateful parts of the environment
        self.first_reset = False  # Built our first world successfully

        # Return an observation
        info['cost'] = cost['cost_sum']
    ...
    def calculate_cost(self):
        return np.array([self._cost()['cost_sum']])
    ...
    def render(self) -> np.ndarray | None:
        """Call underlying :meth:`continuousSafetyGym.safety_gymnasium.bases.underlying.Underlying.render` directly.
