...
from continuousSafetyGym.safety_gymnasium.bases.base_agent import BaseAgent
from continuousSafetyGym.safety_gymnasium.utils.random_generator import RandomGenerator
from continuousSafetyGym.safety_gymnasium.world import Engine
...
class BaseObject(abc.ABC):
    ...
    - :meth:`set_engine`: Set the engine instance, only called once in :class:`continuousSafetyGym.safety_gymnasium.World`.
    ...
    - :attr:`name` (str): Name of the obstacle, used as key in :meth:`process_config` to fill configuration
      dictionary.
    - :attr:`engine` (:class:`continuousSafetyGym.safety_gymnasium.world.Engine`): Physical engine instance.
    - :attr:`random_generator` (:class:`continuousSafetyGym.safety_gymnasium.utils.random_generator.RandomGenerator`):
      Random generator instance.
    - :attr:`agent` (:class:`continuousSafetyGym.safety_gymnasium.bases.base_agent.BaseAgent`): Agent instance.
    ...
    def set_engine(self, engine: Engine) -> None:
        ...
        Note:
            ...
            environment is instantiated in :meth:`continuousSafetyGym.safety_gymnasium.World.bind_engine`.
    ...
    def process_config(self, config: dict, layout: dict, rots: float) -> None:
        ...
        Note:
            This method is called in :meth:`continuousSafetyGym.safety_gymnasium.bases.base_task._build_world_config` to
            fill the configuration dictionary which used to generate mujoco instance xml string of
            environments in :meth:`continuousSafetyGym.safety_gymnasium.World.build`.
        """
        if hasattr(self, 'num'):
            assert (
                len(rots) == self.num
...
class Mocap(BaseObject):
    ...
    def move(self) -> None:
        ...
        Note:
            This method is called in :meth:`continuousSafetyGym.safety_gymnasium.bases.base_task.simulation_forward` before a physics
            step is executed.
        """
