...
from continuousSafetyGym.safety_gymnasium.assets.group import GROUP
from continuousSafetyGym.safety_gymnasium.bases.base_object import Geom
...
class Hazards(Geom):  # pylint: disable=too-many-instance-attributes
    ...
    def cal_cost(self):
        ...
        if not self.is_constrained:
            return cost
        cost['cost_hazards'] = 0
        min_dist = -np.inf # will be the cost for distance to closest hazard
        for h_pos in self.pos:
            h_dist = self.agent.dist_xy(h_pos)
            min_dist = max(min_dist, - (h_dist-self.size))
            # pylint: disable=no-member
            if h_dist <= self.size:
                cost['cost_hazards'] += self.cost * (self.size - h_dist)

        cost['cost_hazards_dist'] = min_dist
        return cost
